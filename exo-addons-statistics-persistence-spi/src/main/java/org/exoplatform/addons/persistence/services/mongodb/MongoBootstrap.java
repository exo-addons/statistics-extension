...
public class MongoBootstrap
{
  ...
  private Mongo mongo()
  {
    if (m==null)
    {
      try
      {
        if (PropertyManager.PROPERTY_DB_SERVER_TYPE_EMBED.equals(PropertyManager.getProperty(PropertyManager.PROPERTY_DB_SERVER_TYPE)))
        {
  ...
  public DB getDB(String dbName)
  {
    if (db==null || dbName!=null)
    {
      if (dbName!=null)
        ...
        db = mongo().getDB(PropertyManager.getProperty(PropertyManager.PROPERTY_DB_SERVER_NAME));
      boolean authenticate = "true".equals(PropertyManager.getProperty(PropertyManager.PROPERTY_DB_SERVER_AUTHENTICATION));
      if (authenticate)
      {
        db.authenticate(PropertyManager.getProperty(PropertyManager.PROPERTY_DB_SERVER_USER), PropertyManager.getProperty(PropertyManager.PROPERTY_DB_SERVER_PASSWORD).toCharArray());
      }
